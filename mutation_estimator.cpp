...
void print_usage() {
    cout << "Mutation Rate Estimator\n"
         ...
         << "  -L <length>        Sequence length (number of positions)\n"
         ...
         << "                     FracMinHash or other sketching methods\n\n"
         << "Examples:\n"
         << "  # Using distinct k-mers of t(pp mode)\n"
         << "  mutation_estimator --mode pp -L 1000000 -k 21 -n 15000\n\n"
         << "  # Using k-mer counts of t (pc mode)\n"
         << "  mutation_estimator --mode pc -L 1000000 -k 21 -n 18000\n\n"
         << "  # Using k-mer counts of s and t (cc mode)\n"
         << "  mutation_estimator --mode cc -L 1000000 -k 21 -n 18000 -s seq.fasta\n\n"
         << "  # With FracMinHash sketch (theta = 0.01)\n"
         << "  mutation_estimator --mode pc -L 1000000 -k 21 -n 180 -t 0.01\n";
}
...
struct EstimatorInput {
    ...
    // For --cc mode only
    string s_sequence;  // Original sequence s
};
...
unordered_map<string, int> kmer_counter(const string& s, int k) {
    ...
    for (size_t i = 0; i <= s.length() - k; i++) {
        kmer_count[s.substr(i, k)]++;
    }
    return kmer_count;
}

// Compute sum of occurrence * h1 for all kmers
double compute_sum_occ_h1(const string& s, int k) {
    set<string> kmer_set = kspectrum(s, k);
    unordered_map<string, int> kmer_count = kmer_counter(s, k);
...
double estimate_r_cc(const EstimatorInput& input) {
    ...
    // Compute sum_occ_h1 from s sequence
    double sum_occ_h1 = compute_sum_occ_h1(input.s_sequence, input.k);
...
int main(int argc, char* argv[]) {
    ...
    if (input.L <= 0) {
        cerr << "Error: Sequence length -L must be positive\n";
        return 1;
    }
    ...
    // Load sequence for cc mode
    if (mode == "cc") {
        input.s_sequence = read_fasta(s_file);
        if (input.s_sequence.length() < static_cast<size_t>(input.k)) {
            cerr << "Error: Sequence in " << s_file << " is shorter than k-mer size\n";
            return 1;
        }
    }
